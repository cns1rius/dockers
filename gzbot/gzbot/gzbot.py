...
def getNotice(URL, MATCH_ID):
    ...
    dic = {
        "platform-notice": URL + f"/api/game/{MATCH_ID}/notices",
        "platform-events": URL
        + f"/api/game/{MATCH_ID}/events?hideContainer=false&count=2&skip=0",
    }
    try:
        ...
        print(
            "\033[31m[%s] [ERROR]: Please add a new challenge and use it! :(\033[0m"
...
async def sendMessageNotice(URL, MATCH_ID, CQ_PORT, GROUP_NOTICE_ID):
    ...
    try:
        while True:
            ...
            print(
                "\033[33m[%s] [INFO]: Waiting data...\033[0m"
                % (str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))))
            )
            if normalListLen < tmpNormalListLen:
                message = "【比赛公告】\n内容：%s\n时间：%s" % (
                    tmpNormalList[0]["content"],
                    processTime(tmpNormalList[0]["time"]),
                )
                print(
                    "\033[32m[%s] [ANNOUNCEMENTS] Data on receipt of Announcements %s\033[0m"
                    % (
                        str(
                            time.strftime(
                ...
                sendMessage(
                    msg=message, CQ_PORT=CQ_PORT, GROUP_NOTICE_ID=GROUP_NOTICE_ID
                )
            ...
            if newchallengeListLen < tmpNewChallengeInfoLen:
                message = "【新增题目】\n%s\n时间：%s" % (
                    tmpNewChallengeInfo[0]["content"],
                    processTime(tmpNewChallengeInfo[0]["time"]),
                )
                print(
                    "\033[32m[%s] [NEW CHALLENGE] Data on receipt of New Challenge %s\033[0m"
                    % (
                        str(
                            time.strftime(
                                "%Y-%m-%d %H:%M:%S", time.localtime(time.time())
            ...
            if newhintLen < tmpNewHintInfoLen:
                message = "【题目提示】\n%s\n时间：%s" % (
                    tmpNewHintInfo[0]["content"],
                    processTime(tmpNewHintInfo[0]["time"]),
                )
                print(
                    "\033[32m[%s] [NEW HINT] Data on receipt of New Hint %s\033[0m"
                    % (
                        str(
                            time.strftime(
                                "%Y-%m-%d %H:%M:%S", time.localtime(time.time())
            ...
            if firstbloodLen < tmpFirstBloodInfoLen:
                message = "【一血播报】\n%s\n时间：%s" % (
                    tmpFirstBloodInfo[0]["content"],
                    processTime(tmpFirstBloodInfo[0]["time"]),
                )
                print(
                    "\033[32m[%s] [FIRST BLOOD] Data on receipt of First Blood %s\033[0m"
                    % (
                        str(
                            time.strftime(
                                "%Y-%m-%d %H:%M:%S", time.localtime(time.time())
            ...
            if secondbloodLen < tmpSecondBloodInfoLen:
                message = "【二血播报】\n%s\n时间：%s" % (
                    tmpSecondBloodInfo[0]["content"],
                    processTime(tmpSecondBloodInfo[0]["time"]),
                )
                print(
                    "\033[32m[%s] [SECOND BLOOD] Data on receipt of First Blood %s\033[0m"
                    % (
                        str(
                            time.strftime(
                                "%Y-%m-%d %H:%M:%S", time.localtime(time.time())
            ...
            if thirdbloodLen < tmpThirdBloodInfoLen:
                message = "【三血播报】\n%s\n时间：%s" % (
                    tmpThirdBloodInfo[0]["content"],
                    processTime(tmpThirdBloodInfo[0]["time"]),
                )
                print(
                    "\033[32m[%s] [THIRD BLOOD] Data on receipt of First Blood %s\033[0m"
                    % (
                        str(
                            time.strftime(
                                "%Y-%m-%d %H:%M:%S", time.localtime(time.time())
        ...
        print(
            "\033[31m[%s] [ERROR]: Trying to exit !!!\033[0m"
            % (str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))))
        )
        print(e)
...
def main():
 ...
 | |  \/   / /(_|_) /  \/ | | | |_    | |_/ / ___ | |_ 
 | | __   / /     | |     | | |  _|   | ___ \/ _ \| __|
 | |_\ \./ /____ _| \__/\ | | | |     | |_/ / (_) | |_    \033[0m\033[4;37m%s\033[0m\033[01;34m
  \____/\_____(_|_)\____/ \_/ \_|     \____/ \___/ \__|   \033[0m\033[4;37m%s\033[0m\n
    ...
    parser.add_argument("--url", required=True, help="platform url")
    parser.add_argument("--notice", required=True, help="qq notice Group")
    parser.add_argument("--id", required=True, help="race id")
    parser.add_argument("--port", required=True, help="cq port")
    parser.add_argument("--events", help="qq detail group (optional)")
    parser.add_argument("--cookie", help="administrator cookie")
